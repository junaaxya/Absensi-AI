...
@section('header-title', 'Edit Kursus')
@section('header-subtitle', $course->title)

@section('content')
<div class="max-w-3xl mx-auto">
    <a href="{{ route('admin.training.show', $course) }}" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 mb-6 transition-colors">
        <span class="material-icons-round text-lg">arrow_back</span>
        Kembali
    </a>

    <div class="bg-white dark:bg-card-dark rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-8">
        <form method="POST" action="{{ route('admin.training.update', $course) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" required
                        class="w-full px-4 py-3 bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary text-sm dark:text-white">{{ old('description', $course->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="duration_hours" value="{{ old('duration_hours', $course->duration_hours) }}" step="0.5" min="0"
                            class="w-full px-4 py-3 bg-white dark:bg-card-dark border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary text-sm dark:text-white">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @if($course->thumbnail)
                            <p class="text-xs text-slate-500 mb-2">Thumbnail saat ini tersedia. Upload baru untuk mengganti.</p>
                        @endif
                        ...
                        @foreach($roles as $role)
                            <label class="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                <input type="checkbox" name="target_roles[]" value="{{ $role }}"
                                    {{ in_array($role, old('target_roles', $course->target_roles ?? [])) ? 'checked' : '' }}
