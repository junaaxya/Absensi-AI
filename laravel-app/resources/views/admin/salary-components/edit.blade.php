...
@section('header-title', 'Edit Komponen Gaji')
@section('header-subtitle', 'Ubah komponen: ' . $salaryComponent->name)
...
<div x-data="wizardForm()" x-cloak>
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Komponen Gaji</h1>
            <p class="text-sm text-slate-500 mt-0.5">Ubah komponen: <strong>{{ $salaryComponent->name }}</strong></p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.salary-components.update', $salaryComponent) }}" method="POST" @submit="handleSubmit">
        @csrf
        @method('PUT')
...
function wizardForm() {
    return {
        ...
        form: {
            name: '{{ old("name", $salaryComponent->name) }}',
            code: '{{ old("code", $salaryComponent->code) }}',
            type: '{{ old("type", $salaryComponent->type) }}',
            is_taxable: {{ old('is_taxable', $salaryComponent->is_taxable) ? 'true' : 'false' }},
            description: `{{ old("description", $salaryComponent->description ?? "") }}`,
            value_type: '{{ old("value_type", $salaryComponent->is_fixed ? "flat" : "formula") }}',
            default_amount: '{{ old("default_amount", $salaryComponent->default_amount) }}',
            formula: '{{ old("formula", "") }}',
        ...
        autoGenerateCode() {
            // Don't auto-generate on edit unless user clears the code
        },
